    ...
    try {
        ...
        */
        ...
        const products = await executeMongoFind(
            {
                collection: 'ept_product_details_new_supreme_mobiles',
                cmpid
            },
            filter,
            { _id: 0 }
        ...
        if(products.length > 0){
            ...
            products.forEach((arrTmp) => {
                ...
                if (productMap.has(key) && arrTmp['product_url'].includes('https://suprememobiles.in/')) {
            ...
            if(ArrGetProductInfo.length > 0){
                ...
                for (const product of ArrGetProductInfo) {
                    ...
                    if (hostname.includes('suprememobiles')) {
                        try {
                            await page.goto(productUrl, {
                                waitUntil: 'networkidle2',
                                timeout: 50000
                            ...
                            if(await page.$('.m-main-product--wrapper') === null) {
                                ...
                                const result = await page.evaluate(async (productUrl) => {
                                    ...
                                    return {
                                        price: getText('div.product-block-group--price span.m-price-item--regular') || '',
                                        stock: getText('.m-add-to-cart--text') || '',
                                        image: getAttr('div[data-slide-id="zoom"] a', 'href') || '',
                                        review: 0,
                                        rating: 0
                                    };
                                ...
                                if (result.stock) {
                                    ...
                                    if (status.includes('add to cart')) {
                        ...
                        return res.status(400).json({
                            status: false,
                            message: 'Only supreme mobiles URLs supported'
                        });
...
module.exports = { supremeMobilesScraper };