    ...
    try {
        ...
        */
        ...
        const products = await executeMongoFind(
            {
                collection: 'ept_product_details_new_reliancedigital',
                cmpid
            },
            filter,
            { _id: 0 }
        ...
        if(products.length > 0){
            ...
            products.forEach((arrTmp) => {
                ...
                if (productMap.has(key) && arrTmp['product_url'].includes('https://www.reliancedigital.in/')) {
            ...
            if(ArrGetProductInfo.length > 0){
                ...
                for (const product of ArrGetProductInfo) {
                    ...
                    if (hostname.includes('reliancedigital')) {
                        try {
                            ...
                            if(await page.$('.product-description-wrapper') === null) {
                                ...
                                const result = await page.evaluate(() => {
                                    ...
                                    const buttontext = document.querySelector('.add-card-btn .wrapper')?.textContent.trim();
                                    ...
                                    return {
                                        ...
                                        review: product?.aggregateRating?.ratingCount || 0
                                    };
                                });
                        ...
                        return res.status(400).json({
                            status: false,
                            message: 'Only reliancedigital URLs supported'
                        });
