        ...
        */
        ...
        if(products.length > 0){
            ...
            products.forEach((arrTmp) => {
                ...
                if (productMap.has(key) && arrTmp['product_url'].includes('https://www.poorvika.com/')) {
                ...
                for (const product of ArrGetProductInfo) {
                    ...
                    if (hostname.includes('poorvika')) {
                        try {
                            ...
                            if(await page.$('.main-detail_product_container__bFKim') === null) {
                                ...
                                const result = await page.evaluate(() => {
                                    ...
                                    const productData = JSON.parse(document.querySelector('#__NEXT_DATA__').textContent);

                                    const image = productData.props.pageProps.pimData.image.url;
                                    const salePrice = productData.props.pageProps.pimData.prices[0].sp[0].price;
                                    const review = productData.props.pageProps.pimData.review_count;
                                    const rating = productData.props.pageProps.pimData.rating;

                                    const avaialablestatus = document.querySelector('.style_text_stock__eeCR_').textContent.trim();

                                    let availabilityStatus = 'outofstock';

                                    if(avaialablestatus === "In Stock"){
