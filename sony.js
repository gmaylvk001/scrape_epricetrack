...
async function sonyScraper(req, res) {
    ...
    const fetchProductPage = async (url, attempt = 1) => {
        ...
        try {
            ...
            if (response.status < 200 || response.status >= 400) {
                throw new Error(
                    `Sony returned HTTP ${response.status}`
                );
            }

            if (!response.data) {
                throw new Error('Empty response from Sony');
            }
            ...
            console.error(
                `Sony request failed (attempt ${attempt}/${maxAttempts}):`,
                error.message
            );
    ...
    // ---------------------------------------------------------
    // PARSE Sony PRODUCT HTML
    // ---------------------------------------------------------
    

    const parseSonyProduct = (html) => {
        ...
        // =====================================================
        const priceElement = $(
            'div.price-share-block div.product__price div.price div.price__sale p'
        ).first();
        ...
        if (!price) {
            const fallbackPrice = $('div.product__price')
                .first()
                .text()
                .trim();

            const matches = fallbackPrice.match(/₹\s*([\d,]+)/);
        ...
        const notifyMeDiv = $('#notify_btn_div').first();

        if (notifyMeDiv.length > 0) {

            const style = (
                notifyMeDiv.attr('style') || ''
            ).toLowerCase();
        ...
        if (reviewText && reviewText[1]) {
            review = parseInt(
                reviewText[1].replace(/,/g, ''),
                10
            );
        }
        ...
        const ratingText = $('body')
            .text()
            .match(/Customers rate us\s+(\d+(?:\.\d+)?)\s*\/\s*5/i);
    ...
    try {
        ...
        for (
            ...
            if (!hostname.includes('shopatsc.com')) {

                sendSSE('warning', {

                    message:
                        'Only Sony URLs supported',
            ...
            try {
                ...
                console.error(
                    `Error scraping Sony product ${productId}:`,
                    error.message
                );
