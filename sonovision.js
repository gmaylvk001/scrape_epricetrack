...
async function sonovisionScraper(req, res) {
    ...
    */
    ...
    req.on('close', () => {
        ...
        console.log('Sonovision client disconnected');
    ...
    */
    ...
    try {
        ...
        sendEvent('step', {
            step: 'products',
            status: 'running',
            message: 'Fetching Sonovision products...'
        });
        ...
        async function processSingleProduct(product, retryCount = 0) {
            ...
            if (!hostname.includes('sonovision')) {
                sendEvent('product_error', {
                    ...
                    productCode,
                    status: 'error',
                    message: 'Only Sonovision URLs supported'
                });
                failedScrapes++;
                return;
            }
            ...
            try {
                sendEvent('product_step', {
                    ...
                    step: 'page_loading',
                    status: 'running',
                    message: 'Opening Sonovision product page...'
                });
        ...
        */
        ...
        console.error('Sonovision scraper error:', error);

        if (!res.writableEnded) {
            try {
                sendEvent('error', {
                    status: false,
                    message: error.message || 'Sonovision scraping failed'
                });
                res.end();
            } catch (e) {
